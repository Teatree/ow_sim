...
def worker(segment_data, shared_data, results):
    deposit_spawn_data, harvestable_spawn_data, illuvialsCounts, shard_names, shard_amounts_values_int, shard_powers, encounter_types, illuvial_weights, illuvials_list, capture_difficulties, shard_amounts = shared_data
    ...
    for day_key in tqdm(segment_data.items()):
        ...
        shardTypeCounts = ['Shard T0','Shard T1','Shard T2','Shard T3','Shard T4','Shard T5']
        ...
        if day_key[0] != '':  # Skip the segment name
            ...
            while populationCount < population:
                ...
                if segment_name == "Max": 
                    ...
                    while i < num_runs:
                        i += 1
                        regionNames.append(random.choices(region_weighted_types, weights=max_region_weighted_values)[0])
                        regionStages.append(random.choices(region_stage_weighted_types, weights=max_region_stage_weighted_values)[0])
                    ...
                    while i < num_runs:
                        i += 1
                        regionNames.append(random.choices(region_weighted_types, weights=high_region_weighted_values)[0])
                    ...
                    while i < num_runs:
                        i += 1
                        regionNames.append(random.choices(region_weighted_types, weights=moderate_region_weighted_values)[0])
                ...
                regionStages_copy = []
                regionNames_copy = []
                for s in range(0, len(regionStages)):
                    if isZero == False and regionStages[s] == 0:
                        isZero = True
                        regionStages_copy.append(regionStages[s])
                        regionNames_copy.append(regionNames[s])
                        continue
                    if isZero and regionStages[s] == 0 and random.random() < 0.15:
                        regionStages[s] = 1
                        regionStages_copy.append(regionStages[s])
                        regionNames_copy.append(regionNames[s])
                    elif isZero and regionStages[s] == 0:
                        # regionNames.pop(s)
                        # regionStages.pop(s)
                        num_runs -= 1
                    else: 
                        regionStages_copy.append(regionStages[s])
                        regionNames_copy.append(regionNames[s])
                regionNames = regionNames_copy.copy()
                regionStages = regionStages_copy.copy()

                # ILLUVIALS
                if len(invShards) > populationCount:
                    currentShardsInv = invShards[populationCount]
                ...
                for run in simMiningResult:
                    ...
                    ores_for_crafting = currentOresCounts
                    if moreThanZeroOres == True:  
                        if curr_ores_inv_counter < len(invMineables): 
                            ...
                            ores_for_crafting = existing_ores_inv
                        else: 
                            invMineables.append(currentOresCounts)

                crafted_ingots_per_per, oresForCrafting = craft_ingots(ores_for_crafting, ingot_recipe_data, ingot_preference_weights)
                invIngots.append((crafted_ingots_per_per, ores_for_crafting))
        ...
        # total_mineable_counts = mineable_counts.copy()
        # if len(invMineables) > 0:
        #     for m_inv in invMineables: 
        #         for k in m_inv.keys():
        #             if k in total_mineable_counts:
        #                 total_mineable_counts[k] += m_inv[k]
            
        # crafted_ingots, oresForCrafting = craft_ingots(total_mineable_counts, ingot_recipe_data, ingot_preference_weights)

        # craftedIngotCounts = list(crafted_ingots.values())
        # crafted_ingots_counts_array = np.array(craftedIngotCounts)
        total_ingots, total_ores_for_crafting = {}, {}
        if len(invIngots) > 0: 
            crafted_ingots, oresForCrafting = zip(*invIngots)
            total_ingots = crafted_ingots[0]
            total_ores_for_crafting = oresForCrafting[0]
            for i in range(1, len(crafted_ingots)):
                for k in total_ingots.keys(): 
                    if k in crafted_ingots[i].keys():
                        total_ingots[k] += crafted_ingots[i][k]
                for k in total_ores_for_crafting.keys(): 
                    if k in oresForCrafting[i].keys():
                        total_ores_for_crafting[k] += oresForCrafting[i][k]
                        
        crafted_ingots_counts_array = np.array(list(total_ingots.values()))
        ...
        if day_key[0] != '':  # Skip the segment name
            simDayData = [day, segment_name, population, total_crypton_spent, runs, regionsAB, regionsBS, regionsCW, regionsS0, regionsS1, regionsS2, regionsS3, extractionsCount, *dpeositCounts, 
                                ...
                                *harvestResultCountsVals, *essenceResultCountsVals, *encounterResults, crafted_ingots_counts_array.sum().item(), 0, *list(total_ingots.values()), str(total_ores_for_crafting), str(invShards), str(invIngots)]
...
def createMiningSimData(num_runs, regionStages, regionNames):
    runsCounter = 0
    simMiningResult = []
    ...
    while runsCounter < num_runs:
        region_name = regionNames[runsCounter]
        region_stage = regionStages[runsCounter]
        runsCounter+=1 
        extractions = []
        extractionCounter = 1
        energyToMine = energyForMining
...
def createHarvestingSimData(num_runs, regionStages, regionNames):
    runsCounter = 0
    simHarvestingResult = []
